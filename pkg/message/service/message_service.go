...
func (m *messageService) MarkRead(data *MarkReadStruct, instance *instance_model.Instance) (string, error) {
	...
	// Em grupo o remetente é o participante dono da mensagem; em conversa
	// individual o próprio chat responde como remetente
	sender := jid
	if strings.Contains(jid.String(), "@g.us") {
		...
		sender = utils.CanonicalJID(participant)
	}
