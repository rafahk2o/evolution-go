...
function loadWorklet() {
  ...
  const source = fs.readFileSync(path.join(__dirname, "..", "audio-worklet.js"), "utf8");
...
test("capture emits 20 ms signed 16-bit PCM frames and supports mute", () => {
  const Capture = loadWorklet()["wacalls-capture"];
  const capture = new Capture();
  const samples = new Float32Array(128).fill(0.5);
  for (let index = 0; index < 8; index += 1) capture.process([[samples]], [], {});
  ...
  const previous = capture.port.messages.length;
  capture.port.onmessage({ data: { enabled: false } });
  for (let index = 0; index < 8; index += 1) capture.process([[samples]], [], {});
  assert.equal(capture.port.messages.length, previous);
});

test("playback converts incoming 16 kHz PCM into output samples", () => {
  const Playback = loadWorklet()["wacalls-playback"];
  const playback = new Playback();
  playback.port.onmessage({ data: new Int16Array(320).fill(16384).buffer });
